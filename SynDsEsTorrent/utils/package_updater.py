...
import os.path
import tarfile
from package_release import borrar_contenido_carpeta

__author__="David"
__date__ ="$15-jul-2014 3:10:46$"

ruta_base = os.path.join('..', 'updater')
ruta_lanz = os.path.join(ruta_base, 'release')
...
def construir_paq_base():
    construir_tar_gz(ruta_pq_b, os.path.join(ruta_base, 'install'))
...
def construir_paq():
    ...
    construir_tar_gz(os.path.join(ruta_lanz, 'SynDsEsTorrent updater ' + version + ' Beta.spk'), 
