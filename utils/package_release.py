    ...
    for dir in os.listdir(os.path.join('..', 'modules')):
        ...
        if os.path.isdir(ruta_mod):
            ...
            with zipfile.ZipFile(nombre_fic, 'w') as myzip:     
                ...
                if host:
                    myzip.write(os.path.join(ruta_host, 'release', basename(ruta_mod) + '.host'), basename(ruta_mod) + '.host')
                if dlm:
                    myzip.write(os.path.join(ruta_dlm, 'release', basename(ruta_mod) + '.dlm'), basename(ruta_mod) + '.dlm')
