...
def obt_ruta_fic(addon, tipo):   
    ...
    if tipo == 'dlm':
        ...
        fichero_base = os.path.join(obt_ruta_base(addon), 'dlm', 'search.php')
    elif tipo == 'host':
        fichero_info = os.path.join(obt_ruta_base(addon), 'host', 'INFO')
        fichero_base = os.path.join(obt_ruta_base(addon), 'host', 'host.php')
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        for dir in os.listdir(os.path.join('..', 'modulos')):
            if os.path.isdir(os.path.join('..', 'modulos', dir)):
